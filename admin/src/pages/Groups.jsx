...
const Groups = () => {
  ...
  const [form, setForm] = useState({ name: '', description: '', educationType: '' });

  const load = async () => {
    try { const gData = await adminApi.getGroups(); setGroups(gData.groups); }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowModal(false); setEditGroup(null); setForm({ name: '', description: '', educationType: '' }); load();
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-3xl font-bold text-white">Groups</h1><p className="text-gray-400 mt-1">Manage groups by education type</p></div>
        <button onClick={() => { setEditGroup(null); setForm({ name: '', description: '', educationType: '' }); setShowModal(true); }}
        ...
        {groups.map((group) => (
          <motion.div key={group._id} whileHover={{ y: -2 }} className="glass-card p-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1"><h3 className="text-white font-semibold text-lg">{group.name}</h3><p className="text-gray-500 text-xs">{group.educationType || 'Unknown'}</p>{group.description && <p className="text-gray-400 text-sm mt-1">{group.description}</p>}</div>
            </div>
            <div className="flex gap-2 mt-4 pt-4 border-t border-white/10">
              <button onClick={() => { setEditGroup(group); setForm({ name: group.name, description: group.description || '', educationType: group.educationType || '' }); setShowModal(true); }}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="glass-card max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div><label className="block text-sm text-gray-300 mb-2">Education Type</label><select value={form.educationType} onChange={(e) => setForm({...form, educationType: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-primary" required>

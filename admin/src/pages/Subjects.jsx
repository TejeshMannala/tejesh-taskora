...
const Subjects = () => {
  ...
  const [form, setForm] = useState({ name: '', description: '', course: '', group: '', color: '#7c3aed' });

  const load = async () => {
    try { const [sData, cData] = await Promise.all([adminApi.getSubjects(), adminApi.getCourses()]); setSubjects(sData.subjects); setCourses(cData.courses); }
    catch {} finally { setLoading(false); }
  };

  useEffect(() => { load(); }, []);

  useEffect(() => { if (form.course) adminApi.getGroups(form.course).then((d) => setGroups(d.groups)).catch(() => {}); else setGroups([]); }, [form.course]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowModal(false); setEditSubject(null); setForm({ name: '', description: '', course: '', group: '', color: '#7c3aed' }); load();
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setEditSubject(null); setForm({ name: '', description: '', course: '', group: '', color: '#7c3aed' }); setShowModal(true); }}
        ...
        {subjects.map((subject) => (
          <motion.div key={subject._id} whileHover={{ y: -2 }} className="glass-card p-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1"><h3 className="text-white font-semibold text-lg">{subject.name}</h3><p className="text-gray-500 text-xs">{subject.course?.name} / {subject.group?.name}</p>{subject.description && <p className="text-gray-400 text-sm mt-1">{subject.description}</p>}</div>
            </div>
            <div className="flex gap-2 mt-4 pt-4 border-t border-white/10">
              <button onClick={() => { setEditSubject(subject); setForm({ name: subject.name, description: subject.description || '', course: subject.course?._id || '', group: subject.group?._id || '', color: subject.color || '#7c3aed' }); if (subject.course?._id) adminApi.getGroups(subject.course._id).then((d) => setGroups(d.groups)).catch(() => {}); setShowModal(true); }}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="glass-card max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div><label className="block text-sm text-gray-300 mb-2">Course</label><select value={form.course} onChange={(e) => setForm({...form, course: e.target.value, group: ''})} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-primary" required>
                <option value="">Select course</option>{courses.map((c) => <option key={c._id} value={c._id}>{c.name}</option>)}
              </select></div>
              <div><label className="block text-sm text-gray-300 mb-2">Group</label><select value={form.group} onChange={(e) => setForm({...form, group: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-primary" required disabled={!form.course}>
